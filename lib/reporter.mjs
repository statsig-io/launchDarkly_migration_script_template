...
const getLdFlagUrl = (ldFlag) => {
    return ldFlag.environments
      ? Object.values(ldFlag.environments).map(env => `https://app.launchdarkly.com/${env._site.href}`).join(" ")
      : `https://app.launchdarkly.com${ldFlag._links.selfl.href}`;
  }

/**
 * 
 * @param {{ loadResults: {ldFlag: any, statsigFeatureFlag: any, error: Error}[]}} loadResults 
 */
export async function outputResults({ loadResults }) {
